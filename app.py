...
def camera():
    ...
    while i <= 30:
        ...
        for x, y, w, h in faces:
            ...
            resized = cv2.resize(face_img, (48, 48))  # Resize to match the input size of the model
            # Convert to grayscale
            gray = cv2.cvtColor(resized, cv2.COLOR_BGR2GRAY)
            # Normalize pixel values to range [0, 1]
            gray = gray / 255.0
            # Add batch dimension and channel dimension to match the input shape of the model
            reshaped = np.expand_dims(np.expand_dims(gray, axis=0), axis=-1)
